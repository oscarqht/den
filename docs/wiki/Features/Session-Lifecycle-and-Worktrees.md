...
- Manages session metadata in the local JSON state file and prompt files in `~/.viba`.
...
- `~/.viba/palx-state.json`:
  - `sessions` section for `SessionMetadata`.
  - `sessionLaunchContexts` section for `SessionLaunchContext`.
  - `drafts` section for saved draft metadata.
